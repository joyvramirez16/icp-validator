    ...
    <div className={isPc ? 'p-8' : 'p-4'} style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #2563eb 0%, #6366f1 100%)' }}>
      <div style={{ maxWidth: isPc ? '900px' : '400px', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: isPc ? '3rem' : '2rem' }}>
          ...
          <button
            onClick={() => { setDeviceMode(null); setResult(null); setLinkedinUrl(''); }}
        ...
        <div style={{ background: 'white', borderRadius: '1rem', boxShadow: '0 20px 50px rgba(0,0,0,0.3)', overflow: 'hidden', marginBottom: '2rem' }}>
          ...
          <div style={{ padding: isPc ? '2rem' : '1.5rem', background: '#f9fafb', borderBottom: '2px solid #e5e7eb' }}>
            <form onSubmit={handleScrapeProfile} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                ...
                <input
                  ...
                  required
                />
                <p style={{ fontSize: '0.75rem', color: '#6b7280', margin: '0.5rem 0 1rem' }}>Paste any LinkedIn profile URL</p>
              </div>
              <button
                type="submit"
                disabled={loading}
            ...
            {error && (
              <div style={{ marginTop: '1rem', padding: '1rem', background: '#fef2f2', borderLeft: '4px solid #dc2626', borderRadius: '0.5rem', display: 'flex', gap: '0.75rem' }}>
          ...
          {result && (
            <div style={{ padding: isPc ? '2rem' : '1.5rem' }}>
              ...
              {/* Profile Info - PC: 2 cols, Mobile: 1 col */}
              {isPc ? (
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                <h3 style={{ fontSize: isPc ? '1.125rem' : '1rem', fontWeight: '600', color: '#111827', marginBottom: '1rem', margin: '0 0 1rem' }}>SOP Criteria Check</h3>
              ...
              {/* Save Button */}
              <button
                onClick={handleSaveToGDoc}
                disabled={saving}
                style={{ width: '100%', background: saving ? '#9ca3af' : '#16a34a', color: 'white', padding: isPc ? '1rem' : '0.75rem', border: 'none', borderRadius: '0.5rem', fontWeight: '600', fontSize: isPc ? '1rem' : '0.875rem', cursor: saving ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}
              >
                {saving ? (
                  <>
                    <Loader2 className="w-5 h-5 animate-spin" />
